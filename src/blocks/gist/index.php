...
function coblocks_block_gist_handler( $matches ) {
	...
	// A `#file-` fragment targets a single file within the gist. Only in that case do
	// we consult the GitHub API to resolve the exact (case-sensitive) filename — the
	// fragment is lower-cased, and some gists contain uppercase filenames. A plain
	// gist embed never depends on this external request, so it renders reliably even
	// when the API is unreachable or rate-limited (e.g. in CI).
	if ( ! empty( $gist_file ) ) {
		$path_parts = explode( '/', $gist_path );
		$gist_id    = isset( $path_parts[1] ) ? $path_parts[1] : '';

		// Fetch from GitHub API the list of all files.
